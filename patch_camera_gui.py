...
class CameraPatchGui:
    ...
    def _build_verification_card(self, parent: ttk.Frame) -> None:
        card = self._card(parent, "3", "Chống reset verification", "Chọn resourceverificationinfosetall.assetbundle để tool xoá entry CommonActions khỏi danh sách verification. Giữ nguyên cấu trúc file, chỉ dịch bytes và giảm count.")
        ...
        ttk.Label(
            card,
            text=r"Lấy trong game: Documents\Resources\1.62.1\assetbundle\resourceverificationinfosetall.assetbundle",
            style="Hint.TLabel",
            wraplength=840,
        ).grid(row=4, column=0, sticky="w", pady=(11, 0))
        ttk.Label(
            card,
            text="Tool xoá entry CommonActions bằng cách dịch bytes và giảm array count. File kết quả xuất cạnh CommonActions đã mod.",
            style="Hint.TLabel",
            wraplength=840,
        ).grid(row=5, column=0, sticky="w", pady=(3, 0))
    ...
    def _build_log_card(self, parent: ttk.Frame) -> None:
        card = self._card(parent, "6", "Nhật ký xử lý", "Theo dõi tool đã tìm file, vá XML nào, xoá verification entry và file kết quả nằm ở đâu.")
        self.log = Text(
            card,
            height=8,
            bg=COLORS["terminal"],
    ...
    def _choose_game_app(self) -> None:
        ...
        if path:
            ...
            self._set_status("Đã chọn kgvn.app", "Tool sẽ tự tìm Data\\resources.assets để trích dictionary")

    def _choose_game_assets_file(self) -> None:
        path = filedialog.askopenfilename(
            title="Chọn resources.assets trong kgvn.app",
            filetypes=[
    ...
    def _choose_verification_file(self) -> None:
        ...
        if path:
            ...
            self._set_status("Đã chọn verification", "Tool sẽ xoá entry CommonActions khỏi verification")
    ...
    def _patch_worker(
        ...
        try:
            ...
            result += f"\nMuc camera: {camera_percent}\n"

            if verification_path is not None:
                vpath = verification_path.expanduser().resolve()
                # Xoá entry CommonActions
                ...
                result += "\n--- Xoa CommonActions khoi verification ---\n"
                result += strip_result.message + "\n"
                if strip_result.output_path:
                    result += f"File chong reset: {strip_result.output_path}\n"
            else:
                result += "Khong chon verification file — chi mod camera.\n"
